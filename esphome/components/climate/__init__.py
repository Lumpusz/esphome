...
from esphome.const import CONF_AWAY, CONF_BOOST, CONF_SLEEP, CONF_ECO, CONF_FREEZE_PROTECTION, \
    CONF_ID, CONF_INTERNAL, CONF_MAX_TEMPERATURE, \
...
CLIMATE_CONTROL_ACTION_SCHEMA = cv.Schema({
    ...
    cv.Optional(CONF_FREEZE_PROTECTION): cv.templatable(cv.boolean),
...
def climate_control_to_code(config, action_id, template_arg, args):
    ...
    if CONF_FREEZE_PROTECTION in config:
        template_ = yield cg.templatable(config[CONF_FREEZE_PROTECTION], args, bool)
        cg.add(var.set_freeze_protection(template_))
